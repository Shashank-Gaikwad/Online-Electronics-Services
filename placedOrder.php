		 ...
		 if(isset($_GET['id']))
		 {
			 if($_GET['id']=="delete")
			 {
				 ...
				 <div class='alert alert-success text-center'>
					 <strong>Success!</strong> Your oder cancelled successfully.
				 </div>
				 ";
			 }
		 }
  ...
  ?>
    ...
    while($row=$r->fetch_assoc())
    {
  	...
  	//$cartQuery="select product.pno,name,image,price,quantity from product,bill_product,bill where bill.cno=$cno and product.pno=bill_product.pno and bill_product.order='0'";
		$cartQuery="select product.pno,product.name,image,product.price,quantity,amount,cartId from product,cart,customer where product.pno=cart.pno and customer.cno=cart.cno and order_status=1 and customer.cno=$cno";
        ...
        echo"<tbody>";
          for($i=0;$i<$cartCount;$i++)
          {
            echo"<tr>";
              ...
              echo"<td>"."<a href='bill.php?q=$cartId_array[$i]' class='btn btn-default btn-lg' role='button'><span class='glyphicon glyphicon-edit'></span>Display Bill</a>"."</td>";
              //echo"<td>"."<a href='deleteOrder.php?q=$cartId_array[$i]&q1=$pno&q2=$cno' class='btn btn-default btn-lg' role='button'><span class='glyphicon glyphicon-trash'></span>Delete</a>"."</td>";
            echo"</tr>";
          }
        echo"</tbody>";
      echo"</table>";
